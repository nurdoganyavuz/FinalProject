...
namespace Core.Utilities.Security.JWT
{
    public class JwtHelper : ITokenHelper
    {
        ...
        public JwtHelper(IConfiguration configuration)
        {
            ...
            _tokenOptions = Configuration.GetSection("TokenOptions").Get<TokenOptions>(); //Configuration(appsettings) içindeki Sectionları gez, TokenOptions ssection'ını getir.
        ...
        {
            _accessTokenExpiration = DateTime.Now.AddMinutes(_tokenOptions.AccessTokenExpiration); //token'ın sona erme süresini tokenoptions'dan aldı
            var securityKey = SecurityKeyHelper.CreateSecurityKey(_tokenOptions.SecurityKey); //token'ı olusturmak için gerekli olan güvenlik anahtarını tokenoptions'dan aldı
