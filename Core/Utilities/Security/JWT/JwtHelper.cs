...
namespace Core.Utilities.Security.JWT
{
    public class JwtHelper : ITokenHelper
    {
        ...
        {
            Configuration = configuration; //bildiğimiz ctor injection
        ...
        {
            var securityKey = SecurityKeyHelper.CreateSecurityKey(_tokenOptions.SecurityKey); //token'ı olusturmak için gerekli olan güvenlik anahtarını _tokenoptions'dan aldı
            ...
            var jwt = CreateJwtSecurityToken(_tokenOptions, user, signingCredentials, operationClaims); //token olusturma
            var jwtSecurityTokenHandler = new JwtSecurityTokenHandler();
            var token = jwtSecurityTokenHandler.WriteToken(jwt); //verilen parametreler doğrultusunda jwt formatında token'ı yazdırma(token stringini olusturma akjhdhgs gibi bir şey)

            return new AccessToken //bir accesstoken döndürmemiz gerekli bu operasyonda
            {
        ...
        private IEnumerable<Claim> SetClaims(User user, List<OperationClaim> operationClaims) //user bilgisini ve claimleri veriyoruz. bu bilgilere göre jwt içerisine claimleri ekleyebilcek.
        {
